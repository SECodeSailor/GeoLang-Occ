...
class DeformableFeatureAggregation(BaseModule):
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        self.last_observation_count = None
        ...
        for (
            ...
            if self.use_deformable_func:
                ...
                # Reuse the projection result as observation evidence for
                # UTP-CLG. A camera is counted once if any Gaussian key point
                # falls inside its image. No additional projection is needed.
                self.last_observation_count = (
                    mask.any(dim=-1)
                    .sum(dim=1)
                    .to(instance_feature.dtype)
                    .unsqueeze(-1)
                    .detach()
                )
